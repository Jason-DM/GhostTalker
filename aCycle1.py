...
from utilityFunctions import pairLoader, eegFeatureReducer, balancedMatrix, featureSelect, speedClass, dirClass, dualClass, fsClass
from sklearn.neighbors import KNeighborsClassifier
from sklearn.feature_selection import RFE
from sklearn import svm
...
X = np.genfromtxt('GTP000_Data.csv', delimiter=',')

y = np.genfromtxt('GTP000_Labels.csv', delimiter=',')
print(np.shape(X))
print(np.shape(y))
X = X.astype(float)
y = y.astype(int)
